...
Then open http://localhost:8080. Vite proxies `/api/*` to FastAPI at
...
The human-style browser QA suite runs the real React wizard and FastAPI application
against disposable PostgreSQL/pgvector, Redis, and WireMock services. The browser enters
the onboarding data; the suite then verifies that the same interaction produced the
expected Resend, OpenAI, Tavily, and Telegram requests. GitHub Actions runs these scenarios
for pull requests and again before publishing production images.

Start Rancher Desktop using the `containerd` engine. Install the dependencies and
Chromium once, from the repository root:
...
Then run the browser scenarios from `web/`:
...
```bash
AGENT_E2E_SLOW_MO_MS=750 npm run test:e2e -- --headed
```

`AGENT_E2E_SLOW_MO_MS` defaults to `0` (normal speed). For example, use `1500`
for a 1.5-second delay between browser actions. This also works with `--ui` in
place of `--headed`. Test timeouts increase with the delay; backend operations
and application timers still run at normal speed.

Omit `--headed` for headless execution. The command starts fresh PostgreSQL,
Redis and WireMock containers, waits for readiness, and runs Playwright, which
starts FastAPI and Vite. It removes this run's containers and disposable data
after success, failure or Ctrl+C. Image downloads are cached by Rancher Desktop.
The suite uses synthetic provider credentials and WireMock responses.

The containers, FastAPI and Vite use automatically allocated localhost ports,
so the suite can run alongside your local app and port-forwards. Playwright passes
the selected addresses to the browser, API, CORS configuration and Vite proxy.
For fixed ports, optionally set `AGENT_E2E_API_URL=http://127.0.0.1:18000` and
`AGENT_E2E_WEB_URL=http://127.0.0.1:18081`; those ports must be free.
Existing application servers are never reused. If using Rancher Desktop's Docker engine, set
`AGENT_E2E_CONTAINER_RUNTIME=docker` when running the command.

GitHub Actions supplies its own service containers and runs
`npm run test:e2e:services`. That command is also available for externally managed
local services, configured with `AGENT_WIREMOCK_URL`, `AGENT_E2E_DATABASE_URL` and
`AGENT_E2E_REDIS_URL`; it does not start or remove containers.
...
kubectl port-forward service/support-customer-service-web 8080:8080 \
  --namespace customer-service
```
...
With `WEB_PUBLIC_BASE_URL` unset, Helm derives `http://localhost:8080` from
`web.service.port` in `values.yaml`. Forward the Service to the same local port
(for example, `8080:8080`). The container itself listens on port 80; an explicit
public URL is required when using a different local forwarding port or an ngrok
tunnel. The URL setting does not start a port-forward.
...
For local deployment, an HTTPS `WEB_PUBLIC_BASE_URL` also supplies the webhook
base URL with `/api` appended, so Telegram reaches `/api/webhooks/telegram`
through the same tunnel. The application derives this from `AGENT_WEB_PUBLIC_BASE_URL`
(Helm: `web.publicBaseUrl`); there is no separate webhook URL setting or Secret key.
Plain HTTP local URLs do not enable Telegram webhook registration outside tests.
...
- `telegram_webhook_base_url`: public HTTPS base URL for this app, without a trailing
  slash; the app normally derives this from `AGENT_WEB_PUBLIC_BASE_URL` plus `/api` during
...
- Telegram credentials can be resolved from one Secret reference per tenant. Secret values
  ...
  `AGENT_WEB_PUBLIC_BASE_URL` is an HTTPS URL, onboarding job processing also
...
If `AGENT_WEB_PUBLIC_BASE_URL` / `web.publicBaseUrl` is an HTTPS URL,
the onboarding job registers the Telegram webhook automatically via the web proxy:
...
Use the public frontend origin, without `/api`. The app appends
`/api/webhooks/telegram` and the tenant query parameter. The frontend must proxy
`/api/` to FastAPI, as the Helm nginx configuration does.
...
| `AGENT_OPENAI_BASE_URL` | unset | Optional OpenAI-compatible base URL; primarily used by isolated contract tests |
...
| `AGENT_TELEGRAM_API_BASE_URL` | `https://api.telegram.org` | Telegram Bot API base URL; tests can point this at WireMock |
...
| `AGENT_TAVILY_API_BASE_URL` | `https://api.tavily.com` | Tavily API base URL; tests can point this at WireMock |
| `AGENT_RESEND_API_BASE_URL` | `https://api.resend.com` | Resend API base URL; tests can point this at WireMock |
