...
WEB_API_BASE_URL=/api \
AGENT_WEBHOOK_PUBLIC_BASE_URL=https://your-web-tunnel.ngrok-free.app/api \
./scripts/deploy-local.sh
```
...
- `telegram_webhook_base_url`: public HTTPS base URL for this app, without a trailing
  slash; the app normally uses `AGENT_WEBHOOK_PUBLIC_BASE_URL` during
...
- Telegram credentials can be resolved from one Secret reference per tenant. Secret values
  ...
  `AGENT_WEBHOOK_PUBLIC_BASE_URL` is configured, onboarding job processing also
...
If `AGENT_WEBHOOK_PUBLIC_BASE_URL` / `webhook.publicBaseUrl` is set,
...
| `AGENT_WEBHOOK_PUBLIC_BASE_URL` | unset | Public API base URL used by onboarding jobs to register external webhooks; Helm reads it from key `AGENT_WEBHOOK_PUBLIC_BASE_URL` in Secret `app-configs` by default |
